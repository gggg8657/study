...
def bfs(y, x):
    ...
    while q:
        ...
        # if world[cur_y][cur_x] == 2 or isDoor[cur_y][cur_x]==True: #<- 문에 있음
        #     for _ in range(4):
        ...
        for k in range(4):
            ny, nx = cur_y + dy[k], cur_x + dx[k]
            if inRange(ny, nx) and not visit[ny][nx] and (world[ny][nx] == world[cur_y][cur_x] or isDoor[cur_y][cur_x]==True):
                q.append((ny, nx))
                visit[ny][nx] = True
                result = max(result, ny)
    return result
